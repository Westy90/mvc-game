 ...
 */
$method = $_SERVER["REQUEST_METHOD"]; //Lagrar om det är GET eller POST som används
$path   = getRoutePath(); //Vad som står i URLen så den vet vilken sida som ska routas till (kan vara två olika om det är GET eller POST)

// Load the routes from the configuration file
$router = null;
$dispatcher = FastRoute\simpleDispatcher(function(FastRoute\RouteCollector $router) {
    require INSTALL_PATH . "/config/router.php"; //laddar in routern
});

// Use the router to find the callback for the route path and retrieve
// the response.
$response = null;
$routeInfo = $dispatcher->dispatch($method, $path); //Här läses om det är GET eller POST och vad som står i URLen för att routas vidare
switch ($routeInfo[0]) {
